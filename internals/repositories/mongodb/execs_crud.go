...
func ForgotPasswordDb(ctx context.Context, email string) (string, error) {
	...
	if err != nil {
		return "", utils.ErrorHandler(err, "interal server error")
	}

	resultUrl := fmt.Sprintf("https://localhost:50051/execs/resetpassword/reset/%s", token)
	message := fmt.Sprintf("Forgot your password? Reset your password using the following link: \n%s\nPlease use the reset code: %s along with your request to change password.\nIf you didn't request a password reset, please ignore this email.\nThis link is only valid for %v minutes.", resultUrl, token, mins)

	subject := "Your password reset link"

	m := mail.NewMessage()
	m.SetHeader("From", "schooladmin@school.com") // replace with sender email
